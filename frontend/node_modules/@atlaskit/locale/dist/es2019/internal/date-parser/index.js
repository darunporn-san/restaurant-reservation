...
const DATE_PARSER_REGEX = /(\d+)[^\d]*(\d+)?[^\d]*(\d+)?\.?/; // Internet Explorer returns non-printing characters when formatting a date
...
export const createDateParser = locale => {
  ...
  const dateFormatter = Intl.DateTimeFormat(normalizedLocale); // Generate a date string from a hard coded date, this allows us to determine
  // the year/month/day position for the provided locale.

  const rawDateString = dateFormatter.format(INVARIANT_DATE);
  const shortDate = stripExtras(rawDateString); // Extract the date pieces from the locale formatted date string
  ...
  const formatParts = extractDateParts(formatMatch); // Find the year/month/day positions of the locale formatted invariant date
  ...
  return date => {
    ...
    const dateParts = extractDateParts(dateMatch); // Use the previously extracted year/month/day positions to extract each
    // date piece.
